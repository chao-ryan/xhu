...
public class Messages extends HttpServlet {
    ...
    public void loginRoom(HttpServletRequest request, HttpServletResponse response)
            ...
            for(int i=0;i<vector.size();i++){
                ...
                if(username.equals(vector.elementAt(i))){
                    ...
                    try {
                        ...
                        out.println("<script language='javascript'>alert('该用户已经登录');window.location.href='/jsp/chat_login.jsp';</script>");
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    flag=false;
        ...
        if(flag){
            ...
            try {
                request.getRequestDispatcher("/jsp/login_ok.jsp").forward(request, response);
            } catch (Exception ex) {
                Logger.getLogger(Messages.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    public void sendMessages(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            application.setAttribute("message", sourceMessage);
            request.getRequestDispatcher("/servlet/Messages.servlet?action=getMessages&nocache=" + random.nextInt(10000)).forward(request, response);
        } catch (Exception ex) {
            Logger.getLogger(Messages.class.getName()).log(Level.SEVERE, null, ex);
        }
