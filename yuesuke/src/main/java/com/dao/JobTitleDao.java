...
public class JobTitleDao extends BaseDao<JobTitle> implements Dao<JobTitle> {
     ...
     * @Description: 保存/添加
     * @author dengchao
     * @param jobTitle 需要保存/添加的classRelation对象
     * @return 返回受影响行数
     * @date 2018/3/23
     */
    @Override
    public Integer save(JobTitle jobTitle) {
        return super.sqlSessionTemplate.insert(getMybatisNameSpace()+"save",jobTitle);
    }
    /**
     * @Title: update
     * @Description: 修改/更新
     * @author dengchao
     * @param jobTitle 需要修改/更新的E对象
     * @return 返回受影响行数
     * @date 2018/3/23
     */
    @Override
    public Integer update(JobTitle jobTitle) {
        return super.sqlSessionTemplate.update(getMybatisNameSpace()+"update",jobTitle);
    }
    /**
     * @Title: findById
     * @Description: 根据id查找
